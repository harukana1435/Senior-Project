...
def make_split_files(split_path, feat_folder, feat_suffix):
    ...
    save_train_path = Path(split_path).parent / 'solos_train.txt'
    save_valid_path = Path(split_path).parent / 'solos_valid.txt'
...
class VASSpecs(torch.utils.data.Dataset):
    ...
    def __init__(self, split, spec_dir_path, mel_num=None, spec_len=None, spec_crop_len=None,
        ...
        self.split_path = f'./data/solos_{split}.txt'
...
class VASFeats(torch.utils.data.Dataset):
    ...
    def __init__(self, split, rgb_feats_dir_path, flow_feats_dir_path, keypoint_feats_dir_path, feat_len, feat_depth, feat_crop_len,
        ...
        self.keypoint_feats_dir_path = keypoint_feats_dir_path
    ...
    def __getitem__(self, idx):
        ...
        # also preprocess flow
        if self.keypoint_feats_dir_path is not None:
            keypoint_path = os.path.join(self.keypoint_feats_dir_path.replace('*', cls), f'{vid}{self.feat_suffix}')
            # just a dummy random features acting like a fake interface for no features experiment
            if self.replace_feats_with_random:
                keypoint_feats = np.random.rand(self.feat_len, self.feat_depth//2).astype(np.float32)
            else:
                keypoint_feats = pickle.load(open(flow_path, 'rb'), encoding='bytes')
            # (T, 2*D)
            feats = np.concatenate((rgb_feats, flow_feats, keypoint_feats), axis=1)
            item['file_path_'] = (rgb_path, flow_path, keypoint_path)
...
if __name__ == '__main__':
    ...
    cfg = OmegaConf.load('./configs/solos_transformer.yaml')
